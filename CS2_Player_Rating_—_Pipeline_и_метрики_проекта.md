...
**MVP работает.** Node.js 24+, локальный интерфейс, SQLite, бесплатные коннекторы, исторические встречи, прогноз команд, шесть сравниваемых моделей и временная проверка. Новые разделы: «Скаутинг», «Готовность данных» и «Вклад игроков».
...
| --- | ---: |
| Матчи BO3 | 40 804 |
| Период | 01.01.2024–19.09.2026 |
| Матчи со статистикой игроков | 4 716 — 11,56% всей истории |
| Матчи с ровно 5 уникальными игроками на каждой стороне | 4 504 |
| Уникальные игроки | 2 805 |
| Строки игрок × матч | 47 769 |
| Пропуски K/D/ADR среди имеющихся строк | 0% |
| KAST / первые убийства / первые смерти | 46 306 строк для каждого поля |
| Trade kills / traded deaths после восстановления из кэша | 45 822 строки для каждого поля |
| Flash assists | 1 463 строки |
| Utility damage | Нет данных |
| Карты Mirage с итоговым счётом | 8 542 |
| Импортированные раунды / полные демки Mirage | 0 / 0 |
| Подтверждённая разметка ролей / closing odds | Нет |

Подробная статистика сейчас покрывает апрель–сентябрь 2026 года; за 2024–2025 годы её нет. Поэтому 40 тысяч результатов нельзя выдавать за 40 тысяч матчей для player-level обучения. Покрытие меняется по месяцам; оно не является случайной выборкой. Отсутствие пропусков в загруженных строках не устраняет смещение отбора.
...
| 4b. Проверка против рынка | Модель и очищенные от маржи closing probabilities | Одинаковые исходы, время снимка до старта, одинаковые матчи и неопределённость разницы ошибок | Нет котировок |
...
Срез от 20.09.2026 — 4 971 матч с полными составами, тест 995 матчей с 22.08.2026, λ = 5:
...
ΔBrier = **+0,0040** в пользу командной базы, bootstrap-интервал по матчам **[−0,0030; +0,0112]** накрывает ноль. Вывод: на текущих данных знание пятёрки **не улучшает** предматчевую вероятность; различие статистически неотличимо от нуля, точечная оценка против RAPM. Причины видны в том же отчёте: 299 из 995 тестовых наблюдений содержат игрока без коэффициента, а 1 415 игроков полностью неотличимы от партнёров — они появляются ровно в одних и тех же наблюдениях, и L2 делит общий сигнал поровну. Это не опровержение гипотезы, а следствие 11,5% покрытия статистикой и короткой предыстории составов.
