...
function comparison(rows){return {model:probabilityMetrics(rows,rows.map(r=>r.p)),market:probabilityMetrics(rows,rows.map(r=>r.marketA)),deltaBrier:avg(rows.map(r=>(r.p-r.y)**2-(r.marketA-r.y)**2)),deltaBrier95:blockInterval(rows,r=>(r.p-r.y)**2-(r.marketA-r.y)**2),calibration:{model:calibration(rows,'p'),market:calibration(rows,'marketA')}};}
...
export function joinPredictions(predictions,matches,quotes,{maxAgeMs=3600000}={}){
  ...
  for(const qs of group.values()){
    ...
    const row={...prediction,provider:q.provider,bookmaker:q.bookmaker,startsAt:m.start,settledAt:finalTime(m),decisionAt:prediction.predictedAt,capturedAt:q.capturedAt,oddsA:q.oddsA,oddsB:q.oddsB,marketA:probability.a,rawA:probability.rawA,rawB:probability.rawB,overround:probability.overround,edge:prediction.p-probability.a,closingA:close?.oddsA||null,closingB:close?.oddsB||null,closingCapturedAt:close?.capturedAt||null,teamA:m.teamA.name,teamB:m.teamB.name};
...
export function bettingReport(matches,quotes,{commission=0,fee=0,leadMinutes=15,maxAgeMinutes=60,includeTrades=false}={}){
  ...
  if(!quotes.length)return {status:'needs_odds',coverage,config,books:[],matchedOos:0,note:'Исторические котировки не загружены. ROI, банк и CLV не рассчитаны. Нужен бесплатный ключ OddsPapi или JSON-архив с timestamps.'};
  ...
  if(!through)return {status:'needs_predictions',coverage,config,books:[],matchedOos:0};
